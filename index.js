...
const SBAPI = {
  ...
  cancel: async (params, h) => {
    try {
      ...
      // If the hold is already gone (404), return a failure result code.
      if (error.response && error.response.status === 404) {
  ...
  chkcharge: async (params, h) => {
    ...
    if (items.length === 0) {
      return Boom.notFound('Item not found for this patron (300)')
    }
  ...
  chkhold: async (params, h) => {
    ...
    if (itemStatusData.faultResponse && itemStatusData.faultResponse.string === 'Item not found in catalog') {
      return Boom.notFound('Record not found (301)')
    }
...
const reportRequestHandler = async (request, h) => {
  ...
  if (!reportConfig) {
    return Boom.badRequest('Invalid report type provided (100)')
  }
  ...
  try {
    ...
    return handleIlsWsError(error)
  }
}

/**
 * Centralized error handler for all ILSWS API interactions.
 * Creates user-friendly Boom error responses.
 * @param {Error} error - The error object caught from an Axios request.
 * @returns {Boom} A Boom error object.
 */
function handleIlsWsError (error) {
  // Handle network or DNS errors from Axios
  if (error.code === 'ENOTFOUND') {
    server.log(['error', 'ilsws'], `DNS resolution failed for ${config.ILSWS_HOSTNAME}`)
    return Boom.badGateway(`Cannot connect to the backend service. (400)`)
  }
  if (error.code === 'ECONNABORTED') {
    server.log(['error', 'ilsws'], `Request to ILSWS timed out: ${error.message}`)
    return Boom.gatewayTimeout(`Backend service timed out. (500)`)
  }

  // Handle HTTP error responses from the ILSWS API
  if (error.response) {
    const { status, data } = error.response
    server.log(['error', 'ilsws'], `ILSWS API Error - Status: ${status}, Data: ${JSON.stringify(data)}`)
    if (status === 404) {
      return Boom.notFound('The requested record could not be found. (300)')
    }
  }
  ...
  // Generic fallback error to avoid leaking implementation details
  return Boom.badImplementation('An unexpected internal error occurred. (600)')
}
